...
class SubscriptionService implements SubscriptionServiceContract
{
    ...
    {
        $baseUrl = defined('GD_COMMERCE_WEBHOOK_DELIVERY_BASE_URL') ? GD_COMMERCE_WEBHOOK_DELIVERY_BASE_URL : SiteRepository::getSiteUrl();

        return Url::fromString($baseUrl)
            ->setQueryParameters(new QueryParameters(['mwc-webhooks' => 'commerce']))
            ->toString();
    }
